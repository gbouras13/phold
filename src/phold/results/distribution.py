...
def get_distribution(
    result_tsv: Path, database: Path, database_name: str, output: Path
) -> pd.DataFrame:
    ...
    foldseek_df = foldseek_df.drop(columns=["query"])
    ...
    # this will be for the distributiom
    counts = (
        foldseek_df.groupby(["record_id", "cds_id", "product"])
        .size()
        .reset_index(name="count")
    )
    print(counts)

    counts_tsv: Path = Path(output) / "counts.tsv"

    counts.to_csv(counts_tsv, sep="\t", index=False)

    # this will be for the distributiom
    counts_function = (
        foldseek_df.groupby(["record_id", "cds_id", "function"])
        .size()
        .reset_index(name="count")
    )
    print(counts_function)

    counts_function_tsv: Path = Path(output) / "counts_function.tsv"

    counts_function.to_csv(counts_function_tsv, sep="\t", index=False)

    return foldseek_df


# def calculate_tophits_results(filtered_tophits_df: pd.DataFrame, cds_dict: dict, output: Path ) -> None:
