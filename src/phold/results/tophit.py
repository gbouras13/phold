...
def get_tophits(result_tsv: Path, top_hits_tsv: Path, evalue: float) -> pd.DataFrame:
    ...
    tophits_df["foldseek_eVal"] = tophits_df["foldseek_eVal"].apply(
    ...
    # save tophits
    tophits_df.to_csv(top_hits_tsv, sep="\t", index=False)
...
def parse_tophits(
    ...
    if database_name == "all_phrogs" or database_name == "all_envhogs":
        ...
        filtered_tophits_df[["phrog", "tophit_protein"]] = filtered_tophits_df[
            "target"
        ].str.split(":", expand=True, n=1)
    elif database_name == "all_phrogs_pdb":
        filtered_tophits_df["phrog"] = filtered_tophits_df["target"].str.replace(
            ".pdb", ""
        )
        filtered_tophits_df["tophit_protein"] = None

    # Remove the original 'target' column
    filtered_tophits_df = filtered_tophits_df.drop(columns=["target"])
    filtered_tophits_df["phrog"] = filtered_tophits_df["phrog"].str.replace(
        "phrog_", ""
    )
    ...
    filtered_tophits_df["product"] = filtered_tophits_df["product"].fillna(
